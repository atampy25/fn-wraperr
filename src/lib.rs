//! This crate provides the [`wrap_err`](attr.wrap_err.html) macro for adding extra error
//! information to a function.
//!
//! Works with [`eyre`] and any other error type which
//! provides a `wrap_err` method taking a string.
//!
//! ```
...
//!
//! #[wrap_err("failed to parse config at `{}`", path.as_ref().display())]
...
/// Add wrap_err to errors from a function.
///
...
///     (|| -> anyhow::Result<()> {
///         function_body()
///     })().map_err(|err| err.wrap_err("wrap_err"))
/// }
/// ```
///
...
///
/// ```
/// use fn_error_wrap_err::wrap_err;
///
/// #[wrap_err(move, "wrap_err")]
/// fn returns_reference(val: &mut u32) -> anyhow::Result<&mut u32> {
///     Ok(&mut *val)
/// }
/// ```
#[proc_macro_attribute]
pub fn wrap_err(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let new_body = if input.sig.asyncness.is_some() {
        ...
        quote! {
            ...
            #result.map_err(|#err| #err.wrap_err(format!(#format_args)).into())
        }
    } else {
        ...
        quote! {
            ...
            (#move_token || #return_ty {
                ...
                #body
            })().map_err(|#err| #err.wrap_err(format!(#format_args)).into())
        }
    };
